...
fn parseAndRespond(data: []const u8, out: []u8) ?[]const u8 {
    ...
    if (std.ascii.eqlIgnoreCase(cmd, "ping")) {
        ...
        _ = iter.next() orelse return null; // skip $<len> for key
        const key: []const u8 = iter.next() orelse return null;
        _ = iter.next() orelse return null; // skip $<len> for value
        const val: []const u8 = iter.next() orelse return null;

        const allocator = gpa.allocator();
        const key_copy = allocator.dupe(u8, key) catch return null;
        const val_copy = allocator.dupe(u8, val) catch return null;

        store_mutex.lock();
        defer store_mutex.unlock();
        store.put(key_copy, val_copy) catch return null;
        return "+OK\r\n";
...
pub fn main() !void {
    store = std.StringHashMap([]const u8).init(gpa.allocator());
    defer store.deinit();
